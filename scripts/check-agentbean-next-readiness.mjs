...
export function collectAgentBeanNextReadinessChecks({
  ...
  const checks = [
    ...
    check(
      ...
      hasPrivilegedManualDefaultBranchGuards(workflow),
      'Every manually dispatched job that receives production or npm credentials must require the repository default branch',
    ),
    check(
...
export function hasPrivilegedManualDefaultBranchGuards(workflow) {
  const privilegedJobs = [
    ['deploy', 'railway-next-preflight'],
    ['railway-next-preflight', 'railway-next-env-sync'],
    ['railway-next-env-sync', 'agentbean-next-production-smoke'],
    ['agentbean-next-production-smoke', 'publish'],
    ['publish', 'promote-agentbean-daemon-latest'],
    ['promote-agentbean-daemon-latest', undefined],
  ];
  return privilegedJobs.every(([start, end]) => {
    const block = extractSourceSection(workflow, `\n  ${start}:`, end ? `\n  ${end}:` : undefined);
    if (!block) return false;
    const ifStart = block.indexOf('\n    if:');
