...
export default function TasksPage() {
  ...
  const sendThreadMessage = () => {
    ...
    getWebSocket().emit('message:send', { channelId, body, parentMessageId: threadParentId, artifactIds }, (res?: { ok?: boolean; error?: string }) => {
      if (res?.ok) return;
      appendMessage({
  ...
  return (
    <div className="flex flex-1 overflow-hidden bg-white">
      <main className="flex min-w-0 flex-1 flex-col">
        ...
        {showCreate && (
          <form onSubmit={handleCreate} className="grid shrink-0 grid-cols-[minmax(160px,1fr)_minmax(180px,1.4fr)_150px_150px_minmax(120px,0.8fr)_auto] items-end gap-3 border-b border-neutral-200 bg-neutral-50 px-4 py-3">
            <Field label="标题">
              <input value={createTitle} onChange={(e) => setCreateTitle(e.target.value)} autoFocus placeholder="任务标题" className="h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm outline-none focus:border-neutral-500" />
            </Field>
            <Field label="描述">
              <input value={createDescription} onChange={(e) => setCreateDescription(e.target.value)} placeholder="补充说明" className="h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm outline-none focus:border-neutral-500" />
            </Field>
            <Field label="频道">
            ...
            <Field label="标签">
              <input value={createTags} onChange={(e) => setCreateTags(e.target.value)} placeholder="聊天, 设计" className="h-9 w-full rounded-md border border-neutral-300 bg-white px-3 text-sm outline-none focus:border-neutral-500" />
            </Field>
            <div className="flex items-center gap-1">
              <button type="submit" disabled={!createTitle.trim() || creating} className="h-9 rounded-md bg-neutral-900 px-3 text-sm font-medium text-white hover:bg-neutral-800 disabled:opacity-50">{creating ? '创建中' : '创建'}</button>
...
function StatusButton({ task, open, compact, onOpen, onMove }: { task: Task; open: boolean; compact?: boolean; onOpen: (open: boolean) => void; onMove: (status: TaskStatus) => void }) {
  ...
  return (
    <div className={`relative inline-block ${compact ? '' : 'mt-3'}`} onClick={(event) => event.stopPropagation()}>
      <button onClick={() => onOpen(!open)} className={`inline-flex h-7 items-center gap-1.5 rounded border px-2 text-xs font-semibold ${status.badge}`}>
