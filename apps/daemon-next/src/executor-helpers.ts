...
import { formatDispatchFailureBody } from '../../../packages/contracts/src/index.js';
...
// Classification rules live in contracts so daemon / web / PI share one source of truth.
export function formatCodexExitFailureBody(exitCode: number, rawOutput: string): string {
  ...
  const classifiedBody = formatDispatchFailureBody({
    status: 'failed',
    detail,
  });
  return [
