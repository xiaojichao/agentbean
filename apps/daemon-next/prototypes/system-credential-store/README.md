...
The Linux x64 probe compiles directly against native `libsecret`, starts an isolated Secret Service inside an ephemeral session D-Bus, and uses the default collection. It verifies the same envelope, immutable-generation, non-secret marker, cleanup and Profile-reference invariants without putting secrets in argv, environment variables or files. It then locks the default collection and proves that the background adapter must preflight `secret_collection_get_locked()` and return `locked` without attempting a lookup, because locked collections may expose an empty item list and libsecret can automatically handle prompts. A separate process with no session D-Bus verifies the `backend_unavailable` mapping. Hosted evidence remains partial because the temporary D-Bus/keyring is not the user's real GNOME/KDE systemd user session and cannot prove sleep, logout or reboot recovery.
