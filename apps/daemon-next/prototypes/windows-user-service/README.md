...
The command builds a self-contained throwaway worker and a WiX 4.0.6 per-user MSI, installs under `%LOCALAPPDATA%`, registers and starts the task, exercises manual start, single-instance behavior, active-work drain, durable outbox flush, immediate scheduler stop and bounded crash restart, then uninstalls and removes scratch files. WiX 4 is pinned deliberately so this throwaway validation does not require accepting the WiX 7 OSMF EULA.
