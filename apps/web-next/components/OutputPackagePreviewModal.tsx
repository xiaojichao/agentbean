  ...
  const submitCurrentReturn = useCallback(async () => {
    ...
    try {
      if (!rejectDelivery) {
        ...
        await loadWorkspace();
        onSaved();
        // 原型对齐：退回（含文件级）总是回讨论串继续——预填被退回版本与智能体选择。
        if (reviewThreadRootMessageId) {
          ...
          if (threadReady) {
            onReturnToThread({
              ...
              taskRevision: reviewPackageBasis.taskRevision ?? 0,
              taskAttempt: reviewPackageBasis.taskAttempt ?? 0,
            });
          }
        }
        return;
