...
function AgentMetricsPanel({
  ...
  return (
    <div className="rounded border border-neutral-200 bg-white p-4" data-smoke="agent-metrics-panel" data-agent-id={metrics.agentId}>
...
export default function AgentMetricsPage() {
  ...
  const fetchMetrics = async () => {
    const res = await agentEvents().metrics(metricsTeamId);
  ...
  return (
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" data-smoke="agent-metrics-list">
          {metricsList.map((m) => (
            <AgentMetricsPanel
              key={m.agentId}
