...
export function PiPolicyPanel({ teamId, canManage }: { teamId: string; canManage: boolean }) {
  const [enabled, setEnabled] = useState<boolean | null>(UNKNOWN_PI_POLICY_STATE.autoCoordinationEnabled);
  ...
  useEffect(() => {
    let active = true;
    setEnabled(UNKNOWN_PI_POLICY_STATE.autoCoordinationEnabled);
    setLoading(true);
    setSaving(false);
    setMessage(null);
    void piPolicyEvents().get(teamId).then((result) => {
      if (!active) return;
      const next = piPolicyStateFromResult(result).autoCoordinationEnabled;
      setEnabled(next);
      if (next === null) setMessage({ ok: false, text: result.error ?? '读取 PI 自动协调状态失败' });
      setLoading(false);
    });
    return () => { active = false; };
  }, [teamId]);
  ...
  const toggle = async (next: boolean) => {
    if (!canManage || loading || saving || enabled === null) return;
    setSaving(true);
    setMessage(null);
    ...
    <section className="rounded-lg border border-neutral-200 p-5" data-smoke="settings-pi-policy" data-team-id={teamId}>
      ...
      <label className="flex items-center gap-3 text-sm" data-smoke="settings-pi-auto-coordination">
        <input
          ...
          disabled={loading || saving || enabled === null || !canManage}
        ...
        <span className="flex-1">
          ...
          <span className="mt-0.5 block text-xs text-neutral-500">
            {loading ? '读取中…' : enabled === null ? '状态未知' : enabled ? '已开启' : '已关闭'}
          </span>
        </span>
      </label>
