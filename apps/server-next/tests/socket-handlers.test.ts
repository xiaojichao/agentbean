...
describe('server-next socket handlers', () => {
  test('registers first-slice web events and forwards payloads to use cases', async () => {
    ...
    const app = {
      ...
      createAdminUser: vi.fn(async (payload) => makeSuccess({ payload })),
      updateAdminUser: vi.fn(async (payload) => makeSuccess({ payload })),
      resetAdminUserPassword: vi.fn(async (payload) => makeSuccess({ payload })),
      ...
      getPiConfigurationReadiness: vi.fn(async () => makeSuccess({ readiness: { status: 'ready', diagnosticCode: null } })),
      setEmergencyStop: vi.fn(async (payload) => makeSuccess({ payload })),
      getEmergencyStop: vi.fn(async (_payload) => makeSuccess({})),
      getTeamPiTokenUsage: vi.fn(async (payload) => makeSuccess({ payload })),
    ...
    expect(socket.eventNames()).toEqual([
      ...
      WEB_EVENTS.piProvider.getEmergencyStop,
      WEB_EVENTS.piUsage.getTeamUsage,
      WEB_EVENTS.agentExposure.createDraft,
      WEB_EVENTS.agentExposure.updateDraft,
      WEB_EVENTS.agentExposure.publish,
      WEB_EVENTS.agentExposure.revoke,
      WEB_EVENTS.agentExposure.listRevisions,
      WEB_EVENTS.agentExposure.getActive,
      WEB_EVENTS.agentExposure.upsertRestriction,
      WEB_EVENTS.agentExposure.getTeamCoverage,
      WEB_EVENTS.agentExposure.getCapabilityDirectory,
      WEB_EVENTS.agentExposure.upsertAutoAcceptPolicy,
      WEB_EVENTS.agentExposure.getAutoAcceptPolicy,
      ...
      WEB_EVENTS.project.createStage,
      WEB_EVENTS.project.createStageEdge,
      ...
      WEB_EVENTS.project.exportWorkspace,
      WEB_EVENTS.project.workspaceRevisions,
      ...
      WEB_EVENTS.admin.createUser,
      WEB_EVENTS.admin.updateUser,
      WEB_EVENTS.admin.resetUserPassword,
      ...
      WEB_EVENTS.memory.formalList,
      WEB_EVENTS.memory.formalDetail,
      WEB_EVENTS.memory.formalCreate,
      WEB_EVENTS.memory.formalRevise,
      WEB_EVENTS.memory.formalDeactivate,
      WEB_EVENTS.memory.formalDelete,
      WEB_EVENTS.memory.proposeCorrection,
      WEB_EVENTS.memory.formalAccept,
      WEB_EVENTS.memory.formalReject,
      WEB_EVENTS.systemKnowledge.list,
      WEB_EVENTS.systemKnowledge.detail,
      WEB_EVENTS.systemKnowledge.create,
      WEB_EVENTS.systemKnowledge.revise,
      WEB_EVENTS.systemKnowledge.deactivate,
      WEB_EVENTS.systemKnowledge.delete,
      WEB_EVENTS.userMemory.list,
      WEB_EVENTS.userMemory.detail,
      WEB_EVENTS.userMemory.create,
      ...
      WEB_EVENTS.memory.projectionWithdraw,
      WEB_EVENTS.memory.projectionListRevisions,
      WEB_EVENTS.memory.projectionUpsertOptIn,
      WEB_EVENTS.memory.projectionGetConsumable,
      WEB_EVENTS.experiencePack.createDraft,
      WEB_EVENTS.experiencePack.approve,
      WEB_EVENTS.experiencePack.withdraw,
      WEB_EVENTS.experiencePack.markSourceInvalid,
      WEB_EVENTS.experiencePack.listByTeam,
      WEB_EVENTS.experiencePack.getById,
      WEB_EVENTS.experiencePack.recommendToChannel,
      WEB_EVENTS.experiencePack.confirmAttachment,
      WEB_EVENTS.experiencePack.revokeAttachment,
