...
describe('device / agent modification ownership (仅设备拥有者 + 系统管理员)', () => {
  ...
  test('team admin cannot rename another member device', async () => {
    ...
    const res = await app.renameDevice({ userId: adminId, teamId, deviceId: deviceAId, hostname: 'hacked' });
    expectForbidden(res);
  });
  ...
  test('system admin can rename another member device', async () => {
    ...
    const res = await app.renameDevice({ userId: 'sysadmin', teamId, deviceId: deviceAId, hostname: 'by-admin' });
    expectOk(res);
  });
  ...
  test('device owner can rename own device', async () => {
    ...
    const res = await app.renameDevice({ userId: ownerId, teamId, deviceId: deviceAId, hostname: 'renamed' });
    expectOk(res);
  });
  ...
  test('plain member cannot rename another member device (守卫)', async () => {
    ...
    const res = await app.renameDevice({ userId: memberId, teamId, deviceId: deviceAId, hostname: 'x' });
    expectForbidden(res);
  });
});
