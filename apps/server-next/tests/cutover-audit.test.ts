    ...
    const checks = collectAgentBeanNextCutoverAudit({
      runCommand: createFakeRunCommand({
        ...
        npmVersions: {
          ...
          '@agentbean/pi-management-runtime@0.1.2': '0.1.2',
          '@agentbean/daemon-next@0.3.10': '0.3.10',
          '@agentbean/daemon@0.3.10': '0.3.10',
        },
        distTags: {
          '@agentbean/daemon': { latest: '0.3.10', legacy: '0.1.35' },
        },
      }),
    });
  ...
  test('can audit CI-provided production environment without GitHub CLI variable or secret listing', () => {
    const checks = collectAgentBeanNextCutoverAudit({
      ...
      runCommand: (_command, args) => {
        ...
        if (args[0] === 'view') {
          if (args[2] === 'dist-tags') {
            return `${JSON.stringify({ latest: '0.3.10', legacy: '0.1.35' })}\n`;
          }
