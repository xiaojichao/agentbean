...
describe('AgentBean Next readiness checker', () => {
  ...
  test('fails closed when the Phase 0 root or CI gate is incomplete', () => {
    const valid = {
      scripts: {
        ...
        'test:server-next-ci': 'cd apps/server-next && ../../node_modules/.bin/vitest run tests --config vitest.config.ts --api.host 127.0.0.1 --exclude tests/phase-2-managed-team-smoke.test.ts --exclude tests/phase-4-managed-server-worker-smoke.test.ts',
    ...
    for (const bypass of [
      ...
      { workflow: `${valid.workflow}\nrun: |\n  npm run test:phase0` },
      { workflow: `${valid.workflow}\nrun: npm run-script test:phase0` },
      { scripts: { ...valid.scripts, 'build:packages': 'npm run build:server-next' } },
  ...
  test('fails closed when a Phase 2 boundary or ordered CI gate is bypassed', () => {
    ...
    expect(hasPhase2TaskDagCiGate({
      scripts: { ...scripts, 'test:packages': 'echo ok' },
      workflow,
    })).toBe(false);
    expect(hasPhase2TaskDagCiGate({
      scripts: { ...scripts, 'build:packages': 'npm run build:server-next' },
      workflow,
    })).toBe(false);
  });
  ...
  test('fails closed when Phase 0 Server management boundaries are bypassed', () => {
    ...
    for (const bypass of [
      ...
      { serverSource: "import { createManagementRuntimeFactory } from '../../../../packages/pi-management-runtime/src/pi-session-adapter.js';" },
      { serverSource: "import type { ManagementSession } from '@agentbean/pi-management-runtime';" },
