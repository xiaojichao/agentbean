...
describe('AgentBean Next browser smoke script', () => {
  ...
  test('频道 Tasks browser smoke 覆盖统一项目工作台（空泳道）、深链回落与无手动配置入口', () => {
    const source = readFileSync(new URL('../../../scripts/smoke-agentbean-next-browser.mjs', import.meta.url), 'utf8');
    ...
    expect(smoke).toContain('non-default channel renders the unified project workbench with empty lanes and no manual stage config entry');
  ...
  test('exercises WebUI workspace run detail with full log artifact and source message link', async () => {
    ...
    const runsPageReadyIndex = calls.findIndex(
      (call) => call[0] === 'waitForFunction'
        ...
        && 'expression' in call[1]
        && typeof call[1].expression === 'string'
        && call[1].expression.includes('admin-runs-page'),
    );
    expect(runsPageReadyIndex).toBeGreaterThanOrEqual(0);
    expect(calls).toContainEqual(['reload', undefined]);
    expect(calls).toContainEqual([
      'setInputValue',
    ...
    expect(waitForFunctionCalls.some((call) => call[1].expression.includes('/dashboard/runs'))).toBe(true);
  ...
  test('uploads a channel file through the App Router chat before opening the file library', async () => {
    const { exerciseWebUiChannelFilesBrowserSmoke } = await import('../../../scripts/smoke-agentbean-next-browser.mjs');
    const calls: Array<[string, unknown]> = [];
    const page = {
      async setFileInputFiles(selector: string, files: unknown) {
        calls.push(['setFileInputFiles', { selector, files }]);
      },
      async setInputValue(selector: string, value: string) {
        calls.push(['setInputValue', { selector, value }]);
    ...
    await expect(exerciseWebUiChannelFilesBrowserSmoke({
      page,
      suffix: 'artifact-smoke',
      timeoutMs: 1000,
    })).resolves.toMatchObject({
