...
function getPackagedLatestDaemonVersion(): string | null {
  ...
  // server-next ships the daemon-next package; resolve package.json relative to
  // process.cwd() so it works whether the server runs from source, from the
  // repository root, or from the packaged dist output (dist keeps the server
  // entry under dist/apps/server-next/src while the daemon package.json sits at
