...
-- 分组键 = (team_id, owner_id, LOWER(TRIM(COALESCE(NULLIF(hostname, ''), json_extract(system_info, '$.hostname')))))，
-- 复刻 normalizeDeviceKey/deviceDisplayKey 的归一化与 system_info.hostname 回退：
-- deviceDisplayKey = normalizeDeviceKey(device.name ?? device.systemInfo?.hostname)，
-- 当 hostname 列为空时回退到 system_info JSON 中的 hostname 字段，避免漏掉历史脏数据。
...
FROM (
  SELECT
    id,
    MIN(id) OVER (
      PARTITION BY team_id, owner_id,
        LOWER(TRIM(COALESCE(NULLIF(hostname, ''), json_extract(system_info, '$.hostname'))))
    ) AS canonical_id
  FROM devices
  WHERE (machine_id IS NULL OR profile_id IS NULL)
    AND LOWER(TRIM(COALESCE(NULLIF(hostname, ''), json_extract(system_info, '$.hostname')))) <> ''
) AS grouped
