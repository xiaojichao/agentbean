...
describe('public declaration boundary', () => {
  it('does not export or reference raw PI SDK types', () => {
    ...
    const declarations = [
      'index.d.ts',
      'openai-compatible-management-model-adapter.d.ts',
      'types.d.ts',
    ]
    ...
    expect(declarations).toMatch(/createOpenAiCompatibleManagementModelAdapter/);
    expect(declarations).toMatch(/ManagementModelAdapterErrorCode/);
    ...
    expect(packedFiles).toContain('dist/openai-compatible-management-model-adapter.d.ts');
