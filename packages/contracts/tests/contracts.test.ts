...
describe('first-slice contract result shape', () => {
  ...
  test('exposes first-slice socket event constants without old network naming', () => {
    ...
    expect(WEB_EVENTS.dispatch.cancelChannel).toBe('dispatch:cancel-channel');
  ...
  test('exposes only Team terminology for collaboration-space events', () => {
    ...
    expect(WEB_EVENTS.admin.updateUser).toBe('admin:update-user');
    expect(WEB_EVENTS.admin.resetUserPassword).toBe('admin:reset-user-password');
