...
**Architecture:** 先从共享 contracts 删除兼容事件和旧 DTO 投影，再以同一发布单元原子切换 Server 与 Web；SQLite 只追加迁移，不修改已经执行过的 migration。浏览器旧键在 Release A 完成一次迁移，Release B 在真实 browser/storage 证据成立后删除读取代码。已经退出生产主线的 `apps/server`、`apps/web`、`apps/daemon` 不再继续维护第二套空间模型，而是从 `main` 移除；Server 回滚使用 Git/Railway，Device 回滚使用经 server-next smoke 验证的 canonical daemon-next 已发布版本。npm `legacy=0.1.35` 只作历史归档。
...
- 迁移 helper 只保留到真实旧键 migration、production browser、Device/Artifact/revocation 与 rollback artifact 证据全部通过；Phase -1 完成发布必须移除旧键读取代码及其 allowlist，不设置任意日历等待。
...
| Team 管理页面 | `/:teamPath/networks` | `/:teamPath/teams` | 旧页面实现删除；`next.config.mjs` 提供临时 308 redirect | 真实旧 URL 与 canonical Team route 验证后删除 redirect 和 allowlist | 回滚前一 deployment | redirect、Team create/switch/delete、刷新恢复 |
| Browser storage | `agentbean.networkPath` | `agentbean.teamPath` | 一次读取、写入新键并删除旧键；停止写旧键 | 真实浏览器 storage migration 验证后删除旧键读取和 allowlist | 回滚前一 Web deployment；新键不影响旧部署 | unit test、真实浏览器 storage inspection |
...
| Legacy source | `apps/server`、`apps/web`、`apps/daemon` | `apps/*-next` + `packages/*` | 先验证 Next production 与 server-next-compatible npm artifact，不在 Release A 前删除 | Release B 从 `main` 退役 legacy source | Git/Railway 上一 deployment、经 smoke 的 canonical daemon-next artifact、DB backup | cutover audit、entry/business smoke、package preparation |

兼容策略遵循最小原则：只有浏览器持久化和用户可收藏 URL 在 Release A 暂时保留读取/redirect；一旦真实迁移证据成立，Release B 立即删除，不用固定天数代替风险判断。内部 socket、DTO 与 artifact 写入由同一官方发布单元原子切换，不建立长期双写。旧 Device 或旧 Web 只有在真实活跃客户端证据存在时才允许增加独立、带删除条件的 adapter；不能以“可能有人使用”为由扩大 Phase -1 compatibility surface。
...
- `apps/web-next/lib/team-path.ts`：Release A 提供一次性旧键迁移；Release B 只保留 canonical Team path 新键读写。
...
- `apps/server/`、`apps/web/`、`apps/daemon/`：从 `main` 退役 legacy / rollback 源码；Server 回滚通过 Git/Railway，Device 回滚使用经 server-next smoke 的 canonical npm 已发布包。
...
git commit -m "让 Web 路由和浏览器状态以 Team 为唯一入口" \
  ...
  -m "Confidence: high" \
  -m "Scope-risk: broad" \
  -m "Directive: 真实迁移证据通过后必须删除旧键读取 helper 分支" \
...
**Interfaces:**
- Consumes: Release A 的 cutover、production browser、Device/Artifact/storage/revocation 与 incident 证据均通过；AgentBean Next 已是 production/default、canonical `@agentbean/daemon` 已由 daemon-next 生成、npm `legacy` dist-tag 已存在。
- Produces: `main` 只保留 `apps/*-next` 与 `packages/*` 产品源码；Server rollback 通过 Git/Railway 历史，Device rollback 通过兼容的 canonical daemon-next artifact 完成。

本任务是 **Release B-only broad-risk gate**。Task 编号只用于需求追踪，不代表它在 Release A 前执行；必须在 Task 11 Step 3 的证据门禁通过后、Task 11 Step 4 前执行。
...
- `legacy` 指向已发布的旧 daemon 历史归档；它不兼容 server-next，不计作 rollback artifact。

任一前提失败时停止 Task 8；不得删除 legacy source。
...
- 保留读取 registry `legacy` dist-tag 的历史归档完整性断言，但不再从 `apps/daemon/package.json` 推导旧版本，也不把它描述为回滚能力。

`railway.json` 固定 server-next build/start command。`package.json` 删除 `smoke:agentbean-old-entry` 和只服务 legacy 的 scripts。

执行 `npm install --package-lock-only --ignore-scripts` 更新 root workspace lock，使其不再声明已删除的三个 workspace package；随后使用完整 `npm ci` 构建 root 拥有的 native dependencies，不得改变现有依赖版本。
...
2. Device 客户端回滚使用经 server-next 连接 smoke 验证的 canonical daemon-next 已发布版本；npm `legacy=0.1.35` 只作历史归档；
...
git commit -m "结束 Team path 的一次性浏览器迁移窗口" \
  -m "真实迁移和生产证据完成后删除旧键读取和 checker allowlist，使 Phase -1 最终状态不再保留兼容入口。" \
  -m "Constraint: Release A 风险证据门禁已通过" \
  -m "Confidence: high" \
  -m "Scope-risk: narrow" \
...
6. Task 8 是 **Release B-only broad-risk gate**；只有 Release A 真实风险证据、Next production、npm canonical/legacy truth 和 rollback runbook 都确认后才能执行；不得用任意等待天数替代证据。
