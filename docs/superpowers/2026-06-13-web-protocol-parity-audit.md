...
| 订阅 `teams:snapshot` | `team.snapshot`=`teams:snapshot` | ❓ 静态引用未见 | ❓ | team switch 已实现，但**广播 `teams:snapshot` 的引用未在 transport/ 捕获**——待探针核实是否用别的方式广播 |
...
| `agent:metrics` | `agent.metrics`（已定义） | ❓ 静态引用未见 | ❓ | server-next 可能未实现 metrics 用例 |
| `agents:subscribe` | `agent.subscribe` | ✅ | ✅ | |
| 订阅 `agents:snapshot` | `agent.snapshot` | ✅ | ✅ | |
| 订阅 `agent:status` | `agent.status` | ❓ 静态引用未见 | ❓ | 待核实 server 是否广播 agent 状态 |
| 订阅 `agents:discovered` | `agent.discovered` | ❓ 静态引用未见 | ❓ | 待核实（scanner 发现 agent 的广播） |
...
| 订阅 `tasks:snapshot` / `task:updated` | `task.snapshot` / `task.updated` | ❓ 静态引用未见 | ❓ | server 已处理 task CRUD，但**广播 snapshot/updated 引用未见**——待核实 |
...
| 订阅 `device:status` | `device.status` | ❓ 静态引用未见 | ❓ | 待核实 |
...
### C 类 · contracts 有定义、server 静态引用未见（7 项，待核实）

`teams:snapshot`、`agent:status`、`agents:discovered`、`agent:metrics`、`device:status`、`tasks:snapshot`、`task:updated`

**说明**：本审计的 server-next 数据来自 `transport/` 目录对 `WEB_EVENTS.*.*` 引用模式的静态扫描。这些事件多为 **server → web 的广播**（snapshot/status/discovered）或 metrics 请求，可能：① 在 transport/ 之外用别的方式 emit；② 确未实现。**不能据静态扫描断言「缺失」**，必须在本地对接探针中实测核实。
