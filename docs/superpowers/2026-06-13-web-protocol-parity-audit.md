...
| **B · web 硬编码、contracts 无定义** | 多为 device 管理长尾 + 部分 auth/join/channel | 12 | 逐项决策：补 server 还是裁剪 UI |
...
| `agents:subscribe` | `agent.subscribe` | ✅ | ✅ | 2026-06-15 续：web 订阅 payload 已改为 `{teamId}` |
| 订阅 `agents:snapshot` | `agent.snapshot` | ✅ | ✅ | |
| 订阅 `agent:status` | `agent.status` | ✅ | ✅ | 2026-06-14 续：agent subscriber refresh 后同步广播最新 agent 状态 |
| 订阅 `agents:discovered` | `agent.discovered` | ✅ | ✅ | 2026-06-14 续：`/register` 改走 `device:scan`，daemon 回报 runtimes/agents 后广播扫描结果 |
...
| `channel:create` / `join` / `history` / `message` / snapshot | channel.* | ✅ | ✅ | （组件级使用，server 已实现；2026-06-15 续：`channels:subscribe` payload 已改为 `{teamId}`） |
...
| 订阅 `tasks:snapshot` / `task:updated` | `task.snapshot` / `task.updated` | ✅ | ✅ | `task:updated` 已广播；`tasks:snapshot` 已复用 `channels:subscribe` 的 team 任务上下文广播 |
...
| `devices:subscribe` | —（未定义） | — | ✅ 已移除 | 2026-06-15 续：web 不再发旧 `devices:subscribe`，订阅设备快照复用 `device:list` + `{teamId}` |
...
**处置**：在 `apps/web/lib/socket.ts` 直接改字符串（或改用 `WEB_EVENTS` 常量）。当前已将 contracts 已定义事件迁移到 `@agentbean/contracts` 的 `WEB_EVENTS`；剩余硬编码项对应 contracts 未定义长尾事件，需先决定补 contracts/server 还是裁 UI。

### B 类 · web 硬编码、contracts/server 无定义（12 项）

按子域：

- **Device 长尾（4）**：`device:agents:list`、`device:select-directory`、`device:delete`、`device:rename`
...
| 子域 | 建议处置 |
|---|---|
| Device 长尾 | 多为设备管理 UI 必需；优先补 contracts + server-next 用例（device 是 Next 平台核心差异化能力之一）。`devices:subscribe` 已裁掉，设备订阅复用 `device:list`。 |
...
原始 7 项中，`teams:snapshot`、`task:updated`、`tasks:snapshot`、`agent:status`、`device:status`、`agents:discovered` 已回填广播，`agent:metrics` 已作为 request/ack 实现。当前无 C 类剩余项。
