...
| 订阅 `device:status` | `device.status` | ❓ 静态引用未见 | ❓ | 仍未见 server-next 广播路径；`devices:snapshot` 已实现 |
...
原始 7 项中，`teams:snapshot`、`task:updated`、`agent:status` 已回填广播，`agent:metrics` 已作为 request/ack 实现。当前仍需决策/补齐的剩余项：
