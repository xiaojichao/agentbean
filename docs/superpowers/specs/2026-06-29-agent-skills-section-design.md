...
3. 装新 skill 后，重连设备（hello 首推）或点详情页"刷新"按钮立即触发扫描拉取。**注**：daemon 5 分钟 rescan 是本地 `scan()` + `reportDeviceSnapshot`，不经 `scanRequested`，故**不扫 custom skills**——custom skills 仅在 hello 首推 / web 手动刷新时更新（非 ≤5min 自动）。若未来要 rescan 自动刷新，需让 rescan 触发带 customAgents 的 scanRequested（见"关键风险"）。
...
- **daemon 首次启动拿 custom agent 列表**（已定）：server 在 `device.hello` 成功后主动下发一次 `scanRequested`（含 `customAgents`），触发首次 custom skills 扫描；后续仅 web 手动刷新触发（**daemon 5 分钟 rescan 是本地 scan，不经 scanRequested，不触发 custom skills 扫描——已知 gap**，未来若要自动刷新需让 rescan 触发带 customAgents 的 scanRequested）。统一走 `scanRequested` 通道，不新增 hello ack 字段或 daemon 主动拉事件。
