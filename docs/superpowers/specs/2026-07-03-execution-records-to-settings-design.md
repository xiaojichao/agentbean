...
> 日期：2026-07-03
> 状态：待审阅
> 生产前端：`apps/web-next`（Next.js App Router，动态段 `[networkPath]`）

## 背景

当前「执行记录」(`/{networkPath}/runs`) 作为侧边栏「诊断」分组下的一级入口，存在两个问题：
...
### 设置页（`app/[networkPath]/settings/page.tsx`，665 行）
...
### 执行记录页（`app/[networkPath]/runs/page.tsx`，494 行）
...
| 文件 | 改动 |
|---|---|
| `app/[networkPath]/settings/page.tsx` | `TABS` 数组 + `Tab` 类型加 `'runs'`；import `RunsPanel`；内容区加 `{tab==='runs' && <RunsPanel/>}` |
| `app/[networkPath]/settings/RunsPanel.tsx`（新建） | 迁移 `runs/page.tsx` 主体（标题/过滤器/列表/加载更多），去掉重复 padding |
| `app/[networkPath]/runs/page.tsx` | 列表渲染改为 `redirect` 到 `/{np}/settings`（`runs/[runId]/` 详情页保留不动） |
