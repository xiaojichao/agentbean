...
- 删除非 primary Team 后，Server 选择仍可访问的 fallback Team 并更新当前 Team。
...
- 旧收藏兼容只做 permanent redirect，不保留第二套业务页面；Release B 删除该 redirect 和一次性 key migration。
...
Server 与 Web 直接消费 canonical 字段，不生成第二套空间 projection。Admin Agent 在上述字段上补 `primaryTeamName` 与 owner/device 展示字段。
...
Device 下载附件和上传产物使用 device-bound credential。附件/产物增强失败不应吞掉主 dispatch result；失败需要显式状态或日志证据。
