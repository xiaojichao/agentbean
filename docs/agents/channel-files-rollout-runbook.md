...
## Preview processor 外部依赖

derivative 由子进程生成，Server 以命令方式调用、不链接任何原生库：
...
缺失或不可执行的后果按既有降级语义处理，不会阻断原文件预览/下载：ffmpeg 缺 WebP 编码器时 derivative job 有限重试后 failed；pdftoppm 缺失时 PDF job 直接标记 `unsupported`（错误码 `PREVIEW_PDF_ADAPTER_MISSING`，不重试）。排查时先看 preview job 错误码，再确认对应二进制在 Server 容器 PATH 中。
...
4. 先在单个灰度实例打开 `BROWSER=true`，验收根目录、运行产物、Task/Run/source root、嵌套目录、面包屑、刷新和前进后退。
5. 验收图片、视频首帧、音频、PDF、Markdown 的预览/下载后，再在灰度实例打开 `MARKDOWN_EDITING=true`。
...
`/metricsz` 提供不含文件名、路径和用户数据的 Range 响应数及 shadow compare 次数、缺失、意外和变更计数。另持续观察平台侧上传/下载失败率与字节量、索引查询延迟、preview backlog/时延/失败码、Markdown 冲突率、回填进度、Server RSS、Artifact 与 derivative 存储增长。
