...
每条 command 只有一个绑定版本的 Causation reference，并可带多个 Source references；没有上游领域事实的根 command 引用 Server 在认证入口签发、先于 command identity 存在的 external action/request fact，禁止伪造来源或 command 自引用。引用保存 provenance，不授予权限，来源后续编辑或删除也不静默改写既有事实。Server 按认证与协议、目标作用域、authority 推导、幂等查重、新 command 的精确 preconditions、事务内复验提交、受众裁剪的顺序执行；所有权威 ID、sequence、revision、fencing token 与时间由 Server 生成。并发控制使用 registry 声明的专项 read/write set，Message freshness、Task/DAG/attempt/delivery、attention、policy 与 PI run/lease 分别维护必要 revision，禁止粗粒度 last-write-wins。
