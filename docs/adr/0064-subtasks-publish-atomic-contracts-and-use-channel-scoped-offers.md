...
Server 先用当前 Channel membership、Team visibility、Task/input 权限、operation restriction 与 hard capacity 建立不可覆盖的 eligibility 硬门槛，再由 PI 根据 Agent Exposure Manifest 的 required Capability/Skill 过滤候选，并以可审计的 preferred Skill、Team 经验、负载、deadline 风险和稳定公平 tie-break 排序。用户确认可以授权向 required Skill 状态 unknown 的显式目标请求绑定 task revision 的 per-Task requirement attestation，但不能替 Agent 声明能力、覆盖权限或明确不满足事实。候选唯一、用户明确指定合格 Agent、确定性排序存在明显赢家或敏感上下文要求最小披露时使用 targeted Offer，多个相近候选或负载未知时使用 candidate-set Offer；两者都不能绕过 Agent acceptance。Server 在 Offer 发布和 accept/claim 事务中复验当前资格与容量，并只允许一个 acceptance 原子取得 claim。
