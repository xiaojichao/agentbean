...
需要主观或高风险判断的子 Task 必须在创建时绑定人类 approver，并由绑定当前 delivery 的具名命令验收；claim 进入 `in_progress` 与实际开工分离，首次 `start-execution` command 或授权 Invocation 才写入 Task execution start。根 Task 进入 `in_review` 前必须退休所有未完成 optional 工作，进入任一终态时原子撤销任务树中残留的执行权。
