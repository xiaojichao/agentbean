...
### Fixed
- 发布 `@agentbean/daemon` / `@agentbean/daemon-next` 0.3.39：收紧 Legacy Daemon 进程识别，不再把 `npm install -g @agentbean/daemon@…` 或命令行里偶然出现包路径的 shell 误判为 Legacy（修复 0.3.38 上 `agentbean update` 仍 `LEGACY_RUNTIME_FENCE_ACTIVE` / 无进度条回滚的问题）
- 发布 `@agentbean/daemon` / `@agentbean/daemon-next` 0.3.38：修复 `agentbean update` 自锁导致 `LEGACY_RUNTIME_FENCE_ACTIVE` → `UPDATE_RECOVERY_REQUIRED`（`update` 子命令与 `update.lock` 持有者不再被误判为 Legacy Daemon）；Device Service 优雅停止后强制退出、restart 超时强制 kill，避免进程挂死后设备离线；`agentbean update` 增加 TTY 进度条与任务文字输出
...
- 发布 `@agentbean/daemon` / `@agentbean/daemon-next` 0.3.21：修复 `agentbean update` 换包期间 LaunchAgent KeepAlive 与半安装依赖竞态（ERR_MODULE_NOT_FOUND → UPDATE_RECOVERY_REQUIRED）；更新前卸掉 LaunchAgent，安装后做模块 import 校验，失败附带 error.log 摘要
...
## [Daily 2026-08-30] - 2026-08-30
### 改进
- #all 与私聊的「聊天 / 任务 / 文件」处理方式与普通频道完全一致：任务 tab 统一项目工作台、文件 tab 统一文件库视图，私聊与默认频道同样支持配置项目阶段并形成交付
- 任务 / 文件 tab 空数据形态统一为空泳道工作台与空文件库板，三个筛选下拉居中展示，文件 tab 不再回落纯附件列表
- 频道与私聊会话界面优化：聊天 / 任务 / 文件标签配图标，图片上传并入通用附件入口，「作为任务」移到输入框右侧；Agent 私聊头部改状态圆点与文本，频道头部展示频道描述

### 修复
- 新建频道表单新增「描述」输入框；编辑频道（含 #all 默认频道）填写的描述现在能正常保存并在重新打开时回显

## [Daily 2026-08-29] - 2026-08-29
### 改进
