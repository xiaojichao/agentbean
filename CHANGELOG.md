...
### Fixed
- 发布 `@agentbean/daemon` / `@agentbean/daemon-next` 0.3.25：Hermes/OpenClaw（agentos-hosted）运行产物默认从 `~/.hermes`、`~/.openclaw` 收集进频道文件索引（与 Codex `generated_images` 同款 mtime 窗口语义），并按 adapter 根串行执行 dispatch 防止并发 run 共享产物目录串线；此前修复已合入 main 但版本未 bump，CI 因 already published 跳过 publish
