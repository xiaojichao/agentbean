...
- 更完整的 workspace run 专用页面布局与完整日志存储仍需后续产品切片；跨页面导航、执行命令投影与受限日志摘要排障工具已补第一步。

### P2：后续产品 parity

- 更完整的 workspace run 专用页面布局与完整日志存储/检索。
...
当前不应再从旧 #141-#148 follow-up 清单直接挑“未完成项”开工。P0 生产观察 baseline 已完成，下一步应开新的 scoped issue/PR 继续产品能力或运维增强；若沿当前替换主线推进，优先在更完整的 workspace run 专用页面/完整日志存储、admin/audit 产品面、settings/device 后续页中选一个小切片。
