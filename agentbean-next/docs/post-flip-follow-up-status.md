...
- 团队改名（`team:update`）已在 preview 团队设置面板覆盖；App Router `devices` 已覆盖 device rename/refresh restore；App Router `settings` 已覆盖 account 当前用户身份/logout 入口、browser preferences 持久化/刷新恢复/reset、team update、join link create/revoke 与刷新恢复；App Router `/:teamPath/teams` 已覆盖受控 team create/switch/delete/fallback restore，并补 SQLite delete cascade 回归。
