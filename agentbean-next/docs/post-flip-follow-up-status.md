...
- Daemon onboarding 已有 invite wait/complete、CLI token persistence、多 profile/YAML、profile list/clear/rename CLI、scan summary、scanner parity、reconnect latest-scan snapshot、canonical npm install smoke 与 production smoke 证据；profile lifecycle 与 reconnect snapshot evidence 已进入 readiness gate。剩余风险集中在 auth token refresh/renewal、更完整 reconnect guarantees 与 onboarding UX drill。
