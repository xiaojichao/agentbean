...
- `Publish agent to npm` job 在 `AGENTBEAN_DEPLOY_TARGET=next` 时，会先发布 `@agentbean/contracts`，再发布 `@agentbean/daemon-next`。
...
- npm 上的 `@agentbean/contracts` 与 `@agentbean/daemon-next` 首次真实发布。
