...
| P3-09 | Green | V3 Worker capability/preflight 与四个 Memory tools | Server 已完成 Phase 3 capability/preflight、四个 exact-key handler、root Task coordination、checkpoint 显式 `managementPhase` 与 Memory repository 接线；兼容旧 daemon，只向声明 V3 capability 的 worker 回送 phase。daemon 注册 `[1,2,3]`，按 checkpoint phase 恢复 runtime、向 V3 Session 恢复有效 Capsule IDs，并只向 Phase 3 暴露完整 Memory tool 集。`memory.propose_candidate` 将真实 Agent 来源与 root user 读取权限分离；三类 Memory 写调用以 body-free `memory-tool-completed` receipt 固化完整 request hash 与结果引用，reconnect replay 可恢复 committed 结果且输入漂移 fail closed；`create_capsule` 使用确定性 ID，`link_sources` 重复调用幂等。Phase 2 边界门禁限定检查 Phase 2 分支，避免被 Phase 3 分支误满足。 |
