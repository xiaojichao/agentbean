...
| P3-09 | Yellow | V3 Worker capability/preflight 与四个 Memory tools | slice 1 工具定义 + slice 2a 请求合同层 + slice 2b V3 capability 门禁接线 + slice 2c-step1/step2-prep **handler 接线缺口全部闭合**：memory.search 补 targetAgentId、collaborative 加 linkSources、**propose_candidate 补全 service 所需提议上下文**（targetAgentId/scopeType/scopeRef + proposedSummary；contract + catalog schema + parser + 测试）。可信来源身份 sourceAgentId/sourceInvocationId 不向模型暴露，由 handler 从 Server 已校验的 run/invocation context 派生；其余 requesterUserId/actorId/currentPolicyVersion/now 同样由 handler 从事实源/config/clock 派生。待补 slice 2c-final：4 handler 实现 + dev-server phase3Handlers 接线 + daemon 注册/transport/host 贯通 + readiness 2→3 |
...
| P3-15 | Green | grant 撤销、来源失效、expire/delete 与审计闭环 | revokeGrant 版本链 + expireMemory/deleteMemory 审计（PR#579）；message/task/artifact/workspace-run/invocation 来源失效反应式级联 `memory-source-invalidation-service.ts`（删除 best-effort 触发，本批失效 + 其余事实源可用性复查，无可用来源时主动 expired + system audit；覆盖分次删除、Task 与频道级联删除，双后端 parity）；E2E 闭环已补（`memory-source-invalidation-e2e.test.ts`：单来源全失→过期→inject 拒、多来源分次全失→过期→inject 拒、部分失效→逐来源检查 inject 拒，双后端 parity） |
