...
| P3-09 | Yellow | V3 Worker capability/preflight 与四个 Memory tools | slice 1 工具定义（contracts Phase3 输入 map + parser + catalog schema + types PHASE_3 名单）+ slice 2a 请求合同层：`Phase3MemoryToolRequestV3`/`ResultV3`（managementPhase:3 + 严格 assertExactKeys parser + isPhase3WorkerToolName guard + contracts 测试）。待补 slice 2b：V3 capability 接线（executor phase3 分支修 `2 as const` echo + adapter accept phase3 + scheduler phase3Preflight + supportsManagementPhase(3)）+ 4 handler 接 service + readiness |
