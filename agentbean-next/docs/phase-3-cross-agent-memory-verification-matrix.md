...
| P3-09 | Yellow | V3 Worker capability/preflight 与四个 Memory tools | slice 1 工具定义 + slice 2a 请求合同层 + slice 2b V3 capability 门禁接线（executor phase3 分支 + scheduler phase3Preflight + adapter route PHASE_3 + contracts context 2|3）+ slice 2c-step1 **handler 接线缺口闭合**：memory.search 输入补 `targetAgentId`（contract + catalog schema + parser）+ collaborative service 新增 `linkSources` 方法（sources.create + source-linked 审计 + 授权，双后端 parity 测试）。create_capsule/propose_candidate input 已与 service 对齐。待补 slice 2c-step2：4 handler 实现（接 search/capsule/candidate/collaborative service）+ dev-server phase3Handlers 接线 + daemon 注册/transport/host 贯通 + readiness 2→3 |
