...
> 原因：合同、Domain 安全边界、Server 持久化、协作 Memory 用例层、权限优先检索排序、最小 Capsule 创建、事实源失效处理、Capsule 注入复验、Capsule↔Invocation/checkpoint 绑定接线、Memory Candidate 生命周期与 Phase 3 Memory 工具定义地基已完成；V3 capability/preflight 接线 + handler、explicit-grant Capsule、实际 inject 接线、Device 本地 Memory 注入合并、Web 治理面、socket wiring 与真实跨 Agent 验证尚未完成。Phase 3 runtime 必须保持关闭。
...
| P3-09 | Yellow | V3 Worker capability/preflight 与四个 Memory tools | slice 1 工具定义地基：contracts `Phase3ManagementWorkerToolInputMapV1`（4 工具输入 DTO + parser）+ catalog `phase3MemorySchemaFor`（typebox 输入 schema，扩 schemaFor/execute 解析 phase3）+ types `PHASE_3_MANAGEMENT_TOOL_NAMES`（phase2 + memory.*，index 导出）。待补 slice 2：V3 capability/preflight 接线（adapter/executor/scheduler）+ 4 个 handler 接 search/capsule/candidate service + readiness 2→3 |
