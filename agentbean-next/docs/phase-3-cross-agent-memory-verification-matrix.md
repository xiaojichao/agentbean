...
> 原因：合同、Domain 安全边界、Server 持久化、协作 Memory 用例层（CRUD/状态机/显式共享）、权限优先检索排序、最小 Capsule 创建、事实源失效处理、Capsule 注入复验与 Capsule-Invocation 绑定（Task 6）已完成；explicit-grant Capsule、实际 inject 接线、Candidate、Device 本地 Memory 注入合并、Web 治理面、socket wiring 与真实跨 Agent 验证尚未完成。Phase 3 runtime 必须保持关闭。
...
| P3-08 | Green | Capsule 与 immutable Invocation intent/checkpoint 绑定 | `memoryCapsuleRef` 嵌入 `AgentInvocationIntentV1`（intentHash 天然冻结）；`invokeTask` 透传 + `assertTaskInvocationReplay` 逐字段比对（重放漂移→conflict）；recovery `validMemoryCapsuleIds` 从 invocations 扫 `ref.expiresAt` 轻量过滤（过期剔除=fail closed，来源失效/hash 漂移留 P3-13 inject）；`invalid-memory-capsule` rebuild reason + boundary gate `P3_INVOCATION_CAPSULE_BINDING_INVALID`。Task 6 PR 待合并 |
...
| P3-16 | Yellow | checkpoint/recovery 不恢复无效 Capsule/Candidate | Capsule 过期剔除已实现（recovery `validMemoryCapsuleIds` 只含 `expiresAt > now`，重建 checkpoint 剔除过期引用，Task 6）；Candidate 部分（P3-10/11）未实现 |
