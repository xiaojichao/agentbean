...
本切片不引入 full-text index、ranking、highlight、saved filters；DM 搜索已在后续切片（#272）补齐。
...
- `ServerNextUseCases.searchMessages` 会校验 team membership、最小 query 长度，并搜索当前用户可见的普通 channels 与 DM（DM 搜索由 #272 补齐，遵守 agent 可见性规则）。
