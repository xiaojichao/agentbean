...
- 当前切片：Issue #554，Team 灰度策略与最小 Task DAG Web 界面
- 工具链：Node 24.18.0
...
| P2-17 | Yellow | Task detail 已接入 DAG、dependency、claim/assignee、attempt、acceptance、raw result 入口与默认折叠的管理事件；逐 Task late snapshot revision guard、permission-filtered DAG API 与 delivery-bound evidence refs 已由 Server/Web tests 覆盖，Node 24 下 web-next 208 项全量通过 | Task 12 补真实浏览器 smoke |
