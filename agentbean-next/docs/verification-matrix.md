...
| P3-21 | strict cutover audit 检查 production data dir/entry URL/secrets、canonical npm versions、`latest` daemon-next 与固定 `legacy@0.1.35` 历史归档，不再读取无实际控制作用的 old/next deploy target。 | Release/CI | Release B 后部署目标已经固定，门禁应验证真实外部依赖而不是陈旧开关；旧协议归档不可冒充 Device rollback。 | `production-cutover-runbook.md`, `scripts/audit-agentbean-next-cutover.mjs` |
| P3-22 | CI 的 `AgentBean Next production smoke` job 必须先运行 strict cutover audit，再运行 public entry smoke 与 business smoke。 | Release/CI | 防止 production smoke 在 GitHub secrets、production URL 或 npm registry 状态漂移时假绿。 | `production-cutover-runbook.md`, `.github/workflows/ci-cd.yml` |
| P3-23 | CI 必须阻止手动 `run_production_deploy=true` 但未请求 `run_agentbean_next_production_smoke=true` 的 dispatch；main push 部署后必须自动运行 production smoke。 | Release/CI | 固定 server-next 后仍不能只部署不验证。 | `production-cutover-runbook.md`, `.github/workflows/ci-cd.yml` |
| P3-24 | Server 应用回滚只能使用与当前 SQLite schema 兼容的上一成功 Railway deployment 或 Git revert 后的 AgentBean Next；不得从 `main` 重建 old binary。 | Release/Operations | 缺少 Release A 前 global DB backup，old binary 不能安全读取已迁移 schema。 | `production-cutover-runbook.md` |
| P3-25 | CI 固定从仓库根目录部署 server-next，不提供 old-target input、旧入口 smoke 或 legacy source build。 | Release/CI | 主线只能维护一个产品实现。 | `.github/workflows/ci-cd.yml`, `scripts/check-agentbean-next-readiness.mjs` |
| P3-26 | main release 必须按 validate → publish → deploy → smoke 排序，缺少 `NPM_TOKEN` 或 `RAILWAY_TOKEN` 时 fail closed。 | Release/CI | 不能让 smoke 验证旧实例，也不能让未发布/未部署的 commit 得到绿色结论。 | `.github/workflows/ci-cd.yml`, `scripts/check-agentbean-next-readiness.mjs` |
| P3-27 | daily changelog 只 push 一次 `main`，依赖 main workflow 发布部署，不再额外 dispatch 竞争性第二次 deploy。 | Release/CI | 同一 commit 的双 workflow 会互相取消或重复部署。 | `.github/workflows/daily-changelog.yml`, `scripts/check-agentbean-next-readiness.mjs` |

Release B 回退只依赖 Git、Railway 与 npm 已发布 artifact；旧源码和 old-target workflow 不再属于活动合同。
...
| E2E-11e | `webui-admin-dashboard-business-flow` 在 System Admin Console 入口覆盖 global admin 访问、中栏 teams/users/devices/agents/pi section 与 URL 深链、设备详情 runtime/public agent 投影、owner transfer、Agent owner projection、PI 位于 `dashboard/pi`，以及设置不再作为 PI 主入口（旧 `settings?tab=pi` 重定向到 Console）。 | Browser E2E/CI | Admin Console 不能只证明 `admin:list-*` 或 `admin:transfer-device-owner` socket/usecase 存在；必须证明全局管理员能经中栏切换 section、在 `dashboard/pi` 完成系统 PI 管理入口，且设置侧无 PI 主 Tab。 | `parity-backfill-audit.md`, `post-flip-follow-up-status.md`, `scripts/smoke-agentbean-next-browser.mjs`, `scripts/check-agentbean-next-readiness.mjs` |
