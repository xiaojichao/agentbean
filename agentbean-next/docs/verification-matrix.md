...
P3-24 与 P3-25 记录 final flip 之前的历史回退门禁，不再定义 Phase -1 的当前发布路径。Release A 起旧源码只作为限时回退参考，不参与 build、deploy 或 publish；可执行回退使用 Git 固定提交、Railway 历史 deployment 与 npm 已发布 artifact，Release B 删除旧源码后继续沿用这一边界。
...
| E2E-11b | `webui-agents-business-flow` 在 App Router 智能体入口覆盖 custom agent create、list/detail、config update、publish/unpublish、metrics dispatch 与 delete/list disappearance。 | Browser E2E/CI | 智能体页不能只证明 `agent:create`、`agents:subscribe` 或 metrics API 存在；必须证明用户能在全局 Agent 管理入口完成旧版已有的配置、发布、指标与删除主路径。 | `parity-backfill-audit.md`, `post-flip-follow-up-status.md`, `scripts/smoke-agentbean-next-browser.mjs` |
...
| E2E-11h | `webui-teams-business-flow` 在 canonical `/:teamPath/teams` 入口覆盖 Team create、switch、delete、fallback restore，并在每个状态转换后刷新验证 current Team 持久化；Release A 的历史 Team 管理收藏入口必须返回 308 permanent redirect 且最终落到 `/:teamPath/teams`。 | Browser E2E/CI | settings / teams 不能只证明 `team:create`、`team:switch` 或路由文件存在；必须证明浏览器状态只写 `agentbean.teamPath`，canonical Team 页面可完成管理主路径，且限时兼容入口没有形成第二套业务实现。 | `scripts/smoke-agentbean-next-browser.mjs`, `apps/server-next/tests/browser-smoke-script.test.ts` |

当前 E2E-07、E2E-08、E2E-09 与 E2E-10 已进入 AgentBean Next CI gate。E2E-11 是所有已迁移入口的 backfill 规则：如果发现旧版已存在但 Next 入口缺少同等行为，要先补 regression test 与 readiness/static gate，再把状态写回本矩阵与 `parity-backfill-audit.md`。`webui-channel-members-business-flow` 已覆盖频道成员入口的 App Router browser-level parity：private channel 创建、creator 添加 human member、添加 agent member、移除 human member、`channel:members` projection、private visibility 回收与 mention scope。`webui-teams-business-flow` 已覆盖 Team 管理入口的 App Router browser-level parity：canonical `/:teamPath/teams` create/switch/delete/fallback restore、逐阶段刷新 current Team 持久化，以及 Release A 历史 Team 管理收藏入口的一次性 308 compatibility redirect。`webui-devices-business-flow` 已覆盖设备入口的 App Router browser-level parity：设备列表/详情、运行时投影、自定义 Agent 投影、targeted scan 后 AgentOS 托管 Agent 投影、重命名刷新恢复与删除后列表消失。`webui-agents-business-flow` 已覆盖智能体入口的 App Router browser-level parity：自定义 Agent 创建、列表/详情、配置更新、跨团队发布/取消发布、metrics dispatch 与删除后列表消失。`webui-task-business-flow` 已覆盖任务入口的 App Router browser-level parity：任务创建、状态更新、排序、删除后列表消失与刷新恢复。`webui-runs-business-flow` 已覆盖运行记录入口的 App Router browser-level parity：执行列表、状态/Agent/设备筛选、状态分组、详情路由、刷新恢复、完整日志 artifact、文件树、inline 日志搜索、返回列表与返回触发消息。`webui-settings-business-flow` 已覆盖设置入口的 App Router browser-level parity：account 当前用户身份/logout 入口、browser preferences 持久化/刷新恢复/reset、team rename、join link create/revoke 与刷新恢复。`webui-admin-dashboard-business-flow` 已覆盖 dashboard/admin 入口的 App Router browser-level parity：global admin 访问、teams/users/devices/agents tab、设备详情 runtime/public agent 投影、owner transfer 与 Agent owner projection。`daemon-onboarding-lifecycle-green` 已覆盖设备连接入口的 daemon/CLI/readiness parity：invite wait/complete、saved profile、token refresh persistence、Socket.IO reconnect、latest scan snapshot、targeted scan、custom agent env device-token boundary 与 canonical npm install smoke。后续新增更完整 search、完整 task page、admin audit/member/device、设备接入异常演练等产品切片时，应在本节追加对应入口级 gate，而不是把已有 smoke 误判为覆盖全部旧产品表面。
