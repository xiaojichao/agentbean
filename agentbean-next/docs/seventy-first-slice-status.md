...
- `/join/[token]`
- `/device-login/[code]`
- `/[networkPath]/dashboard`
- `/[networkPath]/chat`
- `/[networkPath]/channels`
- `/[networkPath]/channels/[channelId]`
- `/[networkPath]/channel/[channelId]`
- `/[networkPath]/dm/[dmId]`
- `/[networkPath]/tasks`
- `/[networkPath]/runs`
- `/[networkPath]/runs/[runId]`
- `/[networkPath]/members`
- `/[networkPath]/human/[userId]`
- `/[networkPath]/agents`
- `/[networkPath]/agents/[agentId]`
- `/[networkPath]/agent/[agentId]`
- `/[networkPath]/agents/metrics`
- `/[networkPath]/devices`
- `/[networkPath]/devices/[id]`
- `/[networkPath]/networks`
- `/[networkPath]/settings`
- `/api/networks/[networkId]/artifacts/upload`
...
- 已补第一版登录后页面级 browser smoke：脚本会通过 `/web` socket 创建隔离 session，把 `agentbean.token` / `agentbean.networkPath` 注入真实 Chrome，然后覆盖 `dashboard/chat/tasks/runs/members/devices/settings`。该 smoke 抓到并修复了 `/runs` 页面在 route team 尚未 resolve 时请求旧 `/api/teams/default/workspace-runs` 的 403。
- 已补登录后业务交互 smoke：
  ...
  - `networks` 页面通过真实 UI 创建新团队，执行 `team:switch` 切到新团队并验证当前团队标记，再进入 settings 执行受控 `team:delete`，确认 fallback 回原团队且临时团队从列表消失。
...
- 本轮 `networks` 加深 smoke 同时补齐了 App Router team management 适配：
  - networks 页面为 `team:create` 表单、team 列表、当前团队标记与 `team:switch` 增加稳定 `data-smoke` hooks。
  - `team:switch` 成功后会跳转到目标 team path 的 networks 页，避免 URL route team 与 server current team 脱节。
  - browser smoke 在验证新 team 可切换后通过 settings 危险区域删除该临时 team，确认路由离开被删 team，并在原 team networks 页确认临时 team 不再出现。
...
- 本轮 `channels` 加深 smoke 同时补齐了 canonical payload 适配：
  ...
  - channels 列表页按 route `networkPath` resolve team，并为创建、列表、编辑、归档确认增加稳定 `data-smoke` hooks。
- 本轮 `agents` 加深 smoke 同时补齐了 canonical payload/DTO 适配：
  - server-next agent snapshot 使用 canonical `primaryTeamId/visibleTeamIds`，web-next socket 边界会统一归一化为旧页面仍使用的 `networkId/publishedNetworkIds`，避免 agents 列表因字段漂移空白。
  - `agent:publish` / `agent:unpublish` / `agent:update-config` / `agent:delete` 客户端调用支持显式管理 team id，避免详情页依赖 cached current team。
  - agents 列表、详情、metrics 页按 route `networkPath` resolve team，不再隐式依赖可能滞后的 store `currentTeamId`。
- 本轮 `devices` 加深 smoke 同时抓到并修复了 canonical contract/UI 问题：
  - 设备页不能只按旧 `userId/networkId/hostname` 判断权限与显示名称，需要兼容 server-next canonical `ownerId/teamId/name`。
