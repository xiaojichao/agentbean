...
- `apps/server-next`：SQLite schema/repositories、use cases、Socket.IO namespace adapters、channel controls、channel membership、member detail projections、device detail、device scan routing、channel/agent/device snapshot subscriptions 与 E2E smoke。
- `apps/daemon-next`：daemon protocol client、stub executor、builtin scanner、reconnect re-announcement、device/runtimes reports、targeted rescan command 与 dispatch error/env 边界。
