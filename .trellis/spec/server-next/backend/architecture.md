...
| 单个 subscriber 的 Team access 读取抛异常 | 本次排除该 subscriber、保留 subscription 供后续重试，并继续处理其他合法受众 |
...
- `dispatch-socket-projection.test.ts`：断言顺序、Dispatch/Team 去重、Team 权限复验、撤权清理与单订阅者异常隔离、频道与 DM 可见性、Task/Memory 唯一 owner、失败与缺失 Team 输入。
