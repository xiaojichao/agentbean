...
| 单个 subscriber 的 Team access 读取抛异常 | 本次排除该 subscriber、保留 subscription 供后续重试，记录包含固定 event、Team/User correlation 与 error class 的有界 warning，并继续处理其他合法受众；不得记录原始错误消息或对象 |
...
- `dispatch-socket-projection.test.ts`：断言顺序、Dispatch/Team 去重、Team 权限复验、撤权清理、单订阅者异常隔离与有界诊断、频道与 DM 可见性、Task/Memory 唯一 owner、失败与缺失 Team 输入。
