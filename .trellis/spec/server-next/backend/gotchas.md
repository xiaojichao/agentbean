...
- **历史 publish 补偿**:旧终态结果可能没有 `workspaceRun.publishId`;补偿重放只允许新增这一字段,正文/artifact/workspaceRun 其余字段必须仍匹配首次结果 fingerprint,随后还要过 committed staging 的 team/channel/agent/workspaceRun provenance 绑定才可成包并补写原回复卡片。
